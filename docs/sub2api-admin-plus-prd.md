...
- 前端已提供独立导航模块：
  ...
  - 本地用量
  - 动作建议
- API E2E 脚本覆盖真实 HTTP、真实 PostgreSQL、供应商父子绑定、本地用量读取、账单对账和动作建议。

部分完成：

- 插件任务后端协议已完成，但 Chrome 插件本体和真实网页登录采集未完成。
- 账单对账服务已完成，但供应商账单自动导出未完成，当前支持 API 导入和 E2E 数据夹具验证。
- 动作建议生成已完成，但“管理员确认后调用 Sub2API Admin API 执行动作”未完成。
- 本地 Sub2API DB 只读已完成基础账号/用量读取，但 Redis 并发读取和 Channel Monitor 深度指标适配未完成。

未完成：

- 每 10 分钟真实费率抓取调度器。
- 真实 Chrome 插件登录供应商后台、抓取费率/余额/优惠、导出账单。
- 第三方供应商 Sub2API/New API Admin API client。
- Sub2API Redis 只读并发/窗口成本 adapter。
- 每日自动账单导出和定时对账。
- 通知通道、审计日志和动作执行闭环。
...
- 读取 Redis 并发会话和窗口成本，只读访问，不写 Sub2API key。
- 把本地用量明细接入账单对账默认流程。
