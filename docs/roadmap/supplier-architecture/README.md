...
| [09-phase-closure.md](09-phase-closure.md) | P1/P2 阶段收口基线、关闭证据、发布前验收清单、P1.x/P2.x 后续增强和 P3 不实施边界 |
| [10-release-acceptance-runbook.md](10-release-acceptance-runbook.md) | P1/P2 发布前 A1-A12 人工验收执行手册、失败处置和放行记录模板 |
| [11-post-release-ops-runbook.md](11-post-release-ops-runbook.md) | P1/P2 上线后灰度观察、风险降级、版本回滚和运营兜底手册 |
| [12-release-readiness-snapshot.md](12-release-readiness-snapshot.md) | 当前发布就绪快照，汇总已完成自动校验、待执行 A1-A12、放行判定和下一步 |
| [13-release-acceptance-record-template.md](13-release-acceptance-record-template.md) | A1-A12 发布验收记录模板，用于发布 issue、飞书文档或内部留证 |

## 2. 接手者先读

如果只需要判断当前版本能否结束 P1/P2 第一阶段，按这个顺序读：

1. 读 [09-phase-closure.md](09-phase-closure.md)：确认 P1 可关闭、P2 第一阶段可关闭、P3 不实施，以及哪些 P1.x/P2.x 不阻塞当前版本。
2. 读 [12-release-readiness-snapshot.md](12-release-readiness-snapshot.md)：确认当前证据状态和下一步。
3. 跑 [10-release-acceptance-runbook.md](10-release-acceptance-runbook.md)：按 A1-A12 做发布前人工验收并脱敏留证。
4. 用 [13-release-acceptance-record-template.md](13-release-acceptance-record-template.md) 填写验收记录。
5. 发布或灰度后读 [11-post-release-ops-runbook.md](11-post-release-ops-runbook.md)：按 T+30 分钟、T+2 小时、T+24 小时观察，必要时降级或回滚。
6. 涉及表、导入导出、ER 图或数据流转时读 [08-database-design.md](08-database-design.md)，不要直接改 migration 或 service。

当前放行口径：

- P1 主线：只有 A1-A9 全部通过才可放行。
- P2 第一阶段：A10-A12 必须通过；如果 scoped out，必须记录灰度补验计划。
- P3：本轮不实施，不能把多 Sub2API 实例、外部事件或迁移冲突增强拉回当前验收。
- 当前本地自动校验：已通过；正式发布前仍必须在最终代码基线重跑。
- 当前人工验收状态：A1-A12 待在测试、预发或生产灰度环境执行并留存脱敏证据。
- 发布后：如果运营仍必须回 Sub2API 原后台完成主路径操作，当前版本视为未达到“Admin Plus 日常主操作入口”的目标。
...
阶段关闭证据和发布前验收清单见 [09-phase-closure.md](09-phase-closure.md)，逐项执行手册见 [10-release-acceptance-runbook.md](10-release-acceptance-runbook.md)，上线后观察和回滚手册见 [11-post-release-ops-runbook.md](11-post-release-ops-runbook.md)。后续如果继续做 P1.x/P2.x，必须先确认它不会重新扩大 P1/P2 的关闭标准。
...
4. P1 第一阶段已把本地账号运营写回从 service 层收口为 `Sub2APIRoutingPort`，并已提供分组可用性、账号快照、加入分组和开关调度语义化方法；自动补池和动作建议路径下的坏账号关闭已沿该端口落地。远程写回第一阶段已接入 `RemoteAdminAPIRoutingPort`，通过现有 Sub2API Admin API 管理账号分组和调度开关。P3 多实例本轮不实施；未来若恢复，也必须沿这个端口扩展。
...
13. Admin Plus 必须提供本地账号运营镜像，按供应商、第三方分组、有效倍率、本地调度分组和调度状态快速筛选账号。
14. Sub2API 原后台手工变更必须可同步、可审计、可采纳或恢复；Admin Plus 自动写回前必须重新读取本地状态，避免覆盖应急操作。
15. Admin Plus 数据库设计、ER 图和流程表级读写以 [08-database-design.md](08-database-design.md) 为准。
