...
| P2 | 成本利润、通知矩阵、代理质量、纯度检测和细粒度实测预算字段 | 待补齐，用于支撑模型级运营质量和利润判断 |
