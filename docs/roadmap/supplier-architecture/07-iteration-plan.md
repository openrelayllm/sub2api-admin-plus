...
3. 当前本地同库版本可在 Admin Plus 内写 `accounts/account_groups` 并写入 `scheduler_outbox`；P1 第一阶段已从 service 层收口为 `Sub2APIRoutingPort`，并已支持通过现有 Sub2API Admin API 做远程写回。P3 多实例本轮不实施；未来若恢复该方向，也必须沿同一端口扩展，避免写回分叉。
...
阶段关闭证据和发布前验收清单已经收口到 [09-phase-closure.md](09-phase-closure.md)。当前版本后续只做缺陷修复和验收问题修补；新增能力默认进入 P1.x/P2.x backlog。P3 只保留记录，不进入本轮实施和验收，不再扩大 P1/P2 关闭范围。
...
| P3 | 多 Sub2API 实例、跨实例容量、迁移冲突增强、外部事件适配 | 本轮不实施，不进入本轮实施和验收 |
