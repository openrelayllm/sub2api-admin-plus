...
func TestTokenAuditPayloadsSplitOpenAICacheAndContextReplayProbes(t *testing.T) {
	...
	require.Equal(t, "user", contextInput[0].(map[string]any)["role"])
	require.Equal(t, "assistant", contextInput[1].(map[string]any)["role"])
	require.Equal(t, "user", contextInput[2].(map[string]any)["role"])
