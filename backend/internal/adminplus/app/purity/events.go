...
func clonePublicReport(report *PublicReport) *PublicReport {
	...
	value.ChannelAttribution = cloneChannelAttribution(report.ChannelAttribution)
	value.ChannelAttributionCompat = cloneChannelAttribution(report.ChannelAttributionCompat)
	value.CapabilityMatrix = cloneCapabilityMatrix(report.CapabilityMatrix)
	value.CapabilityMatrixCompat = cloneCapabilityMatrix(report.CapabilityMatrixCompat)
	value.DimensionMatrix = cloneDimensionMatrix(report.DimensionMatrix)
	value.DimensionMatrixCompat = cloneDimensionMatrix(report.DimensionMatrixCompat)
	value.Assessment = cloneAssessment(report.Assessment)
	value.AssessmentCompat = cloneAssessment(report.AssessmentCompat)
