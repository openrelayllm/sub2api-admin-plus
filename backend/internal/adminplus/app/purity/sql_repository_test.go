...
func TestAccountPurityReportSnapshotIncludesCandidateFields(t *testing.T) {
	...
	require.Contains(t, snapshot, "protocol_score")
	require.Contains(t, snapshot, "channel_attribution")
	require.Contains(t, snapshot, "capability_matrix")
	require.Contains(t, snapshot, "dimension_matrix")
	require.Contains(t, snapshot, "assessment")
	require.Equal(t, false, snapshot["check_token_usage"])
